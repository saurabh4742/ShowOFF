...
const Page= () => {
  ...
  const FollowNow=async()=>{
    try {
      ...
      const res = await axios.post("/api/member/followunfollow",{Id:params.id});
      setFollowed(res.data.followed)
  ...
  const UnfollowNow=async()=>{
    try {
      Setfollowloadig(true)
      const res = await axios.delete("/api/member/followunfollow",{data:{Id:params.id}});
      setFollowed(res.data.followed)
      Setfollowloadig(false)
      window.location.reload();
    } catch (error) {
          ...
          <Card className="w-full p-2 flex justify-center items-center flex-col ">
            ...
            <div className="flex gap-1 my-2">
              <Button variant="outline" onClick={()=>{
                router.push(`/member/${params.id}/followers`)
              }} className="gap-1 text-sm ">
                Followers
                <span className="text-bold">{totalfollowers}</span>
              </Button>
              <Button onClick={()=>{
                router.push(`/member/${params.id}/followings`)
              }} variant="outline" className="text-sm gap-1 ">
                Following
                <span className="text-bold">{totalfollowings}</span>
              </Button>
            </div>
            {followed?<Button variant="secondary" onClick={UnfollowNow} disabled={followloading} className="rounded-none px-20"
              >
               {followloading? <Loader/>:"Unfollow"}
              </Button>:<Button onClick={FollowNow} disabled={followloading} className="rounded-none px-20"
              >
              ...
              <div className="sm:flex flex-col justify-evenly gap-4  items-center ">
                ...
                <div className="flex flex-col  justify-center   my-2  gap-2 ">
                  ...
                  <div className="flex justify-center  gap-2 items-center">
                    <a target="_blank" href={githubId}><ImGithub size={30}/></a><a target="_blank" href={linkedinId}><Linkedin size={30}/></a>
                  </div>
                </div>
              </div>
            </CardContent>
